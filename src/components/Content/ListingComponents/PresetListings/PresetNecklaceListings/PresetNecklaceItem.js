...
export default function PendantItem({ item, getFromAmount, parentMetalColor }) {
      ...
      <div className="jewelry-builder--grid-inner">
        ...
        <a href={`${url}/LGC-PJ-${item.stockNumber}${metalCode}`}>
          <div className="jewelry-builder--grid-info">
            <div className="jewelry-builder--grid-name">{item.pendantName}</div>
            <div className="jewelry-builder--grid-about">
              {item.diamondShape} Diamond Pendant in {metal}
            </div>
            <div className="jewelry-builder--grid-price">
              From {cookies.currencyCode}{" "}
              {nfObject.format(getFromAmount(item, "preset-pendant", cookies))}
            </div>
          </div>
        </a>
      </div>
